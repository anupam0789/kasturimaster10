...
class EnquiryController extends Controller
{
    public function rejectCustomer($id){
        ...
        if ($customer->status != 0) {
            return redirect('/admin')->with('error', 'You can not access this page directly');
        }else{
            return view('admin.enquiry.index', ['enquiry_id' => $id]);
        }
    }
    ...
    public function approveCustomer($id){
        ...
        if ($customer->status != 0) {
            ...
            return view('admin.enquiry.approve', ['enquiry_id' => $id]);
        }
    }
    ...
    public function sendStatus(Request $request)
    {
        ...
        $customer = Customer::find($request->input('enquiry_id'));
        ...
        if ($customer) {
            if($request->input('enquiry_status') == 1){
                ...
                DB::statement("UPDATE $newlyCreatedDatabaseName.pms_users 
                ...
                $companyMaster = Companymaster::create($companyMasterData);
            ...
            Mail::to($customerEmail)->send(new ProcessEmail($subject, nl2br($request->input('message'))));
        }
